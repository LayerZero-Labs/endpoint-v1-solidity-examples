 ...
 * @dev estimate send token `_tokenId` to (`_dstChainId`, `_toAddress`)
     * _dstChainId - L0 defined chain id to send tokens too
     * _toAddress - dynamic bytes array which contains the address to whom you are sending tokens to on the dstChain
     * _tokenIdArray - token Id array to transfer
     * _useZro - indicates to use zro to pay L0 fees
     */
    function estimateSendMultiFee(uint16 _dstChainId, bytes calldata _toAddress, uint[] memory _tokenIdArray, bool _useZro) external view returns (uint nativeFee, uint zroFee);
     ...
     * _adapterParams - a flexible bytes array to indicate messaging adapter services
     */
     ...
     * _adapterParams - a flexible bytes array to indicate messaging adapter services
     */
    function sendFrom(address _from, uint16 _dstChainId, bytes calldata _toAddress, uint[] calldata _tokenIdArray, address payable _refundAddress, address _zroPaymentAddress, bytes calldata _adapterParams) external payable;
