...
//        _safeMint(_dstOmnichainNFTAddress, omnichainNFT_tokenId);
    ...
    function sendTokens(uint16 _dstChainId, bytes calldata _toAddress, uint256 _amount, address _zroPaymentAddress, bytes calldata _adapterParam) external override payable {
        _sendTokens(_msgSender(), _dstChainId, _toAddress, _amount, _zroPaymentAddress, _adapterParam);
    }

    function sendTokensFrom(address _from, uint16 _dstChainId, bytes calldata _toAddress, uint256 _amount, address _zroPaymentAddress, bytes calldata _adapterParam) external override payable {
        address spender = _msgSender();
        _spendAllowance(_from, spender, _amount);
        _sendTokens(_from, _dstChainId, _toAddress, _amount, _zroPaymentAddress, _adapterParam);
    }

    function _sendTokens(address _from, uint16 _dstChainId, bytes calldata _toAddress, uint256 _amount, address _zroPaymentAddress, bytes calldata _adapterParam) internal {
        if (isMain) {
        ...
        endpoint.send{value: msg.value}(_dstChainId, trustedSourceLookup[_dstChainId], payload, payable(_from), _zroPaymentAddress, _adapterParam);
