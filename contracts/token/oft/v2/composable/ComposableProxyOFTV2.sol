...
contract ComposableProxyOFTV2 is ComposableOFTCoreV2 {
    ...
    constructor(address _token, uint8 _sharedDecimals, address _lzEndpoint) ComposableOFTCoreV2(_sharedDecimals, _lzEndpoint) {
        innerToken = IERC20(_token);

        (bool success, bytes memory data) = _token.staticcall(
            abi.encodeWithSignature("decimals()")
        );
        require(success, "ComposableProxyOFT: failed to get token decimals");
        uint8 decimals = abi.decode(data, (uint8));

        require(_sharedDecimals <= decimals, "ComposableProxyOFT: sharedDecimals must be <= decimals");
