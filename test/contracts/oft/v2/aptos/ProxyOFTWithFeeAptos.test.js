...
describe("Proxy OFT with fee Aptos", () => {
    ...
    before(async () => {
        ...
        proxyOftFactory = await ethers.getContractFactory("ProxyOFTWithFee")
        proxyOftAptosFactory = await ethers.getContractFactory("ProxyOFTWithFeeAptos")
    ...
    describe("constructor()", () => {
        it("ProxyOFTWithFeeAptos reverts when passing more than 6 shared decimals in constructor", async () => {
            await expect(proxyOftAptosFactory.deploy(erc20.address, aptosSharedDecimals + 1, localEndpoint.address)).to.be.revertedWith(
                "ProxyOFTWithFeeAptos: shared decimals exceed maximum allowed"
            )
        })

        it("ProxyOFTWithFee doesn't revert when passing more than 6 shared decimals passed in constructor", async () => {
            await expect(proxyOftFactory.deploy(erc20.address, aptosSharedDecimals + 1, localEndpoint.address)).to.not.be.reverted
        })
    })

    describe("sendFrom()", () => {
        it("ProxyOFTWithFeeAptos reverts when sending amount is greater than uint64.max", async () => {
            await expect(
                proxyOftAptos.sendFrom(
                    owner.address,
                    remoteChainId,
                    ownerAddressBytes32,
            ...
            ).to.be.revertedWith("ProxyOFTWithFeeAptos: outboundAmount overflow")
        })

        it("ProxyOFTWithFee doesn't revert when sending amount is greater than uint64.max", async () => {
            await expect(
                proxyOft.sendFrom(owner.address, localChainId, ownerAddressBytes32, greaterThanMaxAmount, greaterThanMaxAmount, lzCallParams, {
                    value: nativeFee,
                })
            ).to.not.be.reverted
        })

        it("ProxyOFTWithFeeAptos doesn't revert when sending amount is less than or equal to uint64.max", async () => {
            await expect(
                proxyOftAptos.sendFrom(owner.address, remoteChainId, ownerAddressBytes32, maxAmount, maxAmount, lzCallParams, {
                    value: nativeFee,
                })
            ).to.not.be.reverted
        })
